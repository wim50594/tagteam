...
async def list_projects(
    ...
    if current_user.is_admin:
        ...
        if not project_ids:
            return []
        result = await db.exec(
            select(Project).where(Project.id.in_(project_ids)).order_by(desc(Project.created_at))
        )
        projects = result.all()
    ...
    for p in projects:
        ...
        if my_batch:
            annotations_result = await db.exec(
                ...
                .order_by(Annotation.item_id, desc(Annotation.version))
            )
            labeled_count = 0
            seen: set[str] = set()
...
async def get_progress(
    ...
    annotations_result = await db.exec(
        ...
        .order_by(Annotation.annotator_id, Annotation.item_id, desc(Annotation.version))
    )
...
async def export_project_full(
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        annotations_result = await db.exec(
            select(Annotation)
            .where(Annotation.project_id == project_id)
            .order_by(Annotation.item_id, Annotation.annotator_id, Annotation.version)
        )
        annotations_data = []
        user_map: dict[int, str] = {}
        for ann in annotations_result.all():
